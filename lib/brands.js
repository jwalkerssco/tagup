...
function create(deps) {
  ...
  async function list(session) {
    ...
    return { ok: true, brands, unmatched, counts, canWrite: true, aiOn: !!env.ANTHROPIC_API_KEY, vipDistributorId };
  }
  ...
  function scoreCandidates(label, cands) {
    ...
    const scored = cands.map((c) => {
      ...
      const hits = words.filter((w) => tw.indexOf(w) !== -1).length;
  ...
  function matchFilename(filename, brands) {
    ...
    for (const b of brands) {
      ...
      for (const nm of names) {
        ...
        const score = (exact ? 100 : 0) + words.length * 10 + words.join("-").length;
  ...
  return { list, recognize, find, pick, setStatus, approveConfident, setOverride, scoreCandidates, brandWords, matchFilename, bulkUpload, wikipediaArticleImages, matchVipBrand, expandWords, fetchVipBrands, importVip, brandKeyFor, brandKeyForAsync, attachLogos, rows, normBrand, titleCase, callModel, wikiSearch };
}

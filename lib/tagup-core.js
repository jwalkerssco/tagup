...
(function (root, factory) {
  ...
  function fieldValue(f, req, style) {
    ...
    switch (f.key) {
      ...
      case "account": return req.accountName || "";
  ...
  function renderTemplateTag(req, style, material, opts) {
    ...
    const src = pendingSrc || ((o.templateBase || "/api/asset/ttpl/") + encodeURIComponent(style.templateKey || ""));
    ...
    const inner = fields.filter(function (f) { return f.on; }).map(function (f) {
      ...
      if (spec.image) {
        const isrc = req.brandLogoSrc || ((o.brandBase || "/api/asset/blogo/") + encodeURIComponent(val));
  ...
  function renderTag(req, style, material, opts) {
    ...
    const logoUrl = th.showLogo && style && style.logoKey ? (o.assetBase || "/api/asset/tlogo/") + encodeURIComponent(style.logoKey) : null;
    ...
    const footer = req.accountName && o.showAccount ? '<div class="acct">' + esc(req.accountName) + "</div>" : "";
    ...
    const brand = th.showBrandLogo !== false && req.brandLogoKey
      ? '<img class="blogo" src="' + esc(req.brandLogoSrc || ((o.brandBase || "/api/asset/blogo/") + encodeURIComponent(req.brandLogoKey))) + '" alt="">'
      : "";

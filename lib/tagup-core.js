...
(function (root, factory) {
  ...
  const DEFAULT_THEME = {
    ...
    showItem: true,                      // Modern: the item name line; hidden anyway when the logo already names the product
  ...
  function themeMerge(t) {
    ...
    ["bg", "fg", "accent", "accentFg", "priceColor", "dropColor", "promoColor", "pillColor"].forEach(function (k) {
  ...
  function normalizeField(f) {
    ...
    out.fill = HEX.test(String(b.fill || "")) ? b.fill : null;
  ...
  const DEFAULT_MATERIALS = [
    ...
    { id: "mat_18up_land", name: "Shelf tag 18-up landscape (3x6)", tagW: 3.667, tagH: 1.417, sheetW: 11, sheetH: 8.5, cols: 3, rows: 6, averySku: null },
  ...
  function renderTemplateTag(req, style, material, opts) {
    ...
    const inner = fields.filter(function (f) { return f.on; }).map(function (f) {
      ...
      if (spec.image) {
        ...
        return '<div class="f fimg" style="left:' + f.x + "%;top:" + f.y + "%;width:" + f.w + "%;height:" + f.h + "%" + (f.fill ? ";background:" + f.fill : "") + '"><img src="' + esc(isrc) + '" alt="" style="object-position:' + pos + " " + (f.valign === "top" ? "top" : f.valign === "bottom" ? "bottom" : "center") + '"></div>';
      }
  ...
  function renderTag(req0, style, material, opts) {
    ...
    if (th.layout === "modern") {
      ...
      return '<div class="tag modern" style="--u:' + u.toFixed(3) + ";width:" + W + "in;height:" + H + "in;background:" + bg + ";color:" + fg + ";font-family:" + font.css + ";--head:" + font.head.replace(/"/g, "'") + '">' +
        '<div class="mrow">' + logoBox + '<div class="mcol">' + mitem + mmoney + "</div></div>" +
  ...
  const TAG_CSS =
    ...
    ".tag.modern .mrow{flex:1 1 auto;height:100%;display:flex;align-items:center;gap:calc(.08in*var(--u));padding:calc(.05in*var(--u)) calc(.08in*var(--u));min-height:0;box-sizing:border-box}" +
    ".tag.modern .mlogo{flex:0 0 30%;aspect-ratio:1;max-height:92%;border-radius:50%;overflow:hidden;display:flex;align-items:center;justify-content:center}" +
    ".tag.modern .mlogo img{width:92%;height:92%;object-fit:contain;display:block}" +
    ".tag.modern .mlogo.mini{font-family:var(--head);font-weight:700;font-size:calc(.5in*var(--u));line-height:1}" +
    ...
    ".tag.modern .mitem{font-family:var(--head);text-transform:uppercase;font-weight:700;font-size:calc(.13in*var(--u));line-height:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%;display:flex;gap:calc(.06in*var(--u));align-items:baseline}" +
    ...
    ".tag.modern .mprice .cur{font-size:calc(.24in*var(--u));margin-top:calc(.05in*var(--u))}" +
    ".tag.modern .mprice .whole{font-size:calc(.6in*var(--u))}" +
    ".tag.modern .mprice .cents{font-size:calc(.24in*var(--u));margin-top:calc(.05in*var(--u));margin-left:calc(.012in*var(--u))}" +
    ".tag.modern .mprice .multi{font-size:calc(.42in*var(--u))}" +
    ".tag.modern .mprice.op{font-size:calc(.26in*var(--u));text-transform:uppercase;line-height:1}" +
    ".tag.modern .mpill{color:#fff;font-family:var(--head);font-weight:700;font-size:calc(.11in*var(--u));line-height:1;padding:calc(.035in*var(--u)) calc(.1in*var(--u));border-radius:calc(.14in*var(--u));white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%;margin-top:calc(.02in*var(--u))}" +
    ".tag.modern .msub{font-size:calc(.085in*var(--u));font-weight:600;opacity:.85;margin-top:calc(.02in*var(--u));white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%}" +
    ".tag.modern .msub .was{text-decoration:line-through;opacity:.8}" +
